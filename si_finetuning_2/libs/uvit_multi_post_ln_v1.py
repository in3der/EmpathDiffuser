...
class UViT(nn.Module):
    ...
    def forward(self, img, clip_img, text, t_img, t_text, data_type):
        ...
        text = self.text_embed(text)    # text_dim (64) -> embed_dim (768)
        #print("🦖🦖🦖🦖🦖🦖🦖🦖🦖clip_img shape before Linear:", clip_img.shape)
        clip_img = self.clip_img_embed(clip_img)    # clip_img_dim (512) -> embed+dim (768)
        clip_img = clip_img.unsqueeze(dim=1)  # [B, 768] → [B, 1, 768]  # 차원맞추려고 추가함
